...
inventory_scanners_bp = Blueprint('inventory_scanners', __name__)

@inventory_scanners_bp.route('/get_all', methods=['GET'])
@require_auth
def get_all():
    ...
    try:
        ...
        if include_archived:
            ...
            query = "SELECT * FROM scanners WHERE is_deleted = 0"
        cursor.execute(query)
        columns = [column[0] for column in cursor.description]
        for row in cursor.fetchall():
            d = normalize_row(dict(zip(columns, row)))
            d['device_class'] = 'SCANNER'
            raw_no = str(d.get('name') or '')
            if raw_no.isdigit(): d['pr_no'] = f"TR-{raw_no.zfill(3)}"
            else: d['pr_no'] = raw_no
        ...
        print("PRINTER GET_ALL ERR (scanners):", e)
    finally:
        conn.close()

    return jsonify(all_devices)

@inventory_scanners_bp.route('/update', methods=['POST'])
@require_editor
def update_device():
    try:
        data = request.json
        ...
        cursor.execute("SELECT * FROM scanners WHERE id = ? AND is_deleted = 0", (record_id,))
        columns = [column[0] for column in cursor.description]
        ...
        for field in allowed_fields:
            if field in data:
                ...
                if str(new_val) != str(old_val if old_val is not None else ''):
                    log_change('scanners', record_id, record_label, field, old_val, new_val, changed_by, display_name)
        ...
        query = f"UPDATE scanners SET {', '.join(update_fields)} WHERE id = ?"
        ...
        if 'mahal' in data and new_pc != old_pc:
            if old_pc:
                cursor.execute("UPDATE pcs SET scanner_serial = NULL WHERE pc_no = ?", (old_pc.replace('PC-',''),))
            if new_pc:
                cursor.execute("UPDATE pcs SET scanner_serial = ? WHERE pc_no = ?", (serial, new_pc.replace('PC-','')))
        ...
        print(f"[SCANNER UPDATE ERROR] {e}")
        return jsonify({"error": str(e)}), 500

@inventory_scanners_bp.route('/delete/<int:id>', methods=['DELETE'])
@require_editor
def delete_device(id):
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("UPDATE scanners SET is_deleted = 1, deleted_at = GETDATE() WHERE id = ?", (id,))
        conn.commit()
        conn.close()
...
@inventory_scanners_bp.route('/device/<int:device_id>', methods=['GET'])
@require_auth
def get_device_detail(device_id):
    try:
        ...
        cursor = conn.cursor()
        cursor.execute("SELECT * FROM scanners WHERE id = ?", (device_id,))
        row = cursor.fetchone()
        ...
        print(f"[API ERROR] scanner get_device_detail: {e}")
        return jsonify({"error": str(e)}), 500

@inventory_scanners_bp.route('/check_serial', methods=['POST'])
@require_auth
def check_serial():
    try:
        data = request.json
        ...
        cursor.execute("SELECT id FROM scanners WHERE serial_no = ? AND is_deleted = 0", (serial,))
        found = cursor.fetchone() is not None
                
        conn.close()
        return jsonify({"success": True, "exists": found, "table": 'scanners'})
    except Exception as e:
        return jsonify({"error": str(e)}), 500

@inventory_scanners_bp.route('/auto_register', methods=['POST'])
@require_editor
def auto_register():
    try:
        data = request.json
        ...
        cursor.execute("SELECT MAX(id) FROM scanners")
        max_id = cursor.fetchone()[0] or 0
        new_name = f"TR-{(max_id + 1):03d}"
        
        query = "INSERT INTO scanners (name, serial_no, status, pc_no) VALUES (?, ?, ?, ?)"
