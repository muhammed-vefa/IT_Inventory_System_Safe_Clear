...
class SyncManager:
    ...
    def sync_inventory(self):
        ...
        for r in data:
            ...
            os_val = str(_get(r, ['OS', 'ISLETIM SISTEMI'])).upper()
            win = 1 if 'W' in os_val else 0
            keyos = 1 if 'K' in os_val else 0
            
            cursor = conn.cursor()
            cursor.execute("SELECT id FROM inventory WHERE pc_no=?", (pc_no,))
            row = cursor.fetchone()
            
            if row:
                cursor.execute("""UPDATE inventory SET ip=?, kule=?, mahal_kodu=?, mahal_adi=?,
                                 seri_no=?, mac_adresi=?, windows=?, keyos=? WHERE id=?""",
                                 (ip, kule, m_kodu, m_adi, seri, mac, win, keyos, row[0]))
                stats["updated"] += 1
            else:
                cursor.execute("""INSERT INTO inventory (pc_no, ip, kule, mahal_kodu, mahal_adi,
